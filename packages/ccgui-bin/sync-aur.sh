...
install -Dm644 "${SCRIPT_DIR}/LICENSE" "${WORK_DIR}/${PKGNAME}/LICENSE"

if [[ -z "$(git -C "${WORK_DIR}/${PKGNAME}" status --short -- PKGBUILD .SRCINFO LICENSE)" ]]; then
  printf 'no changes\n'
  exit 0
fi
...
git -C "${WORK_DIR}/${PKGNAME}" add PKGBUILD .SRCINFO LICENSE
