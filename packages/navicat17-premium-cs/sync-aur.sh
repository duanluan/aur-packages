...
PKGNAME="${PKGNAME:-navicat17-premium-cs}"
AUR_REMOTE_URL="${AUR_REMOTE_URL:-ssh://aur@aur.archlinux.org/${PKGNAME}.git}"
AUR_FETCH_URL="${AUR_FETCH_URL:-https://aur.archlinux.org/${PKGNAME}.git}"
...
if [[ -f "${AUR_SSH_KEY}" ]]; then
  export GIT_SSH_COMMAND="ssh -o IdentitiesOnly=yes -i ${AUR_SSH_KEY}"
fi

"${SCRIPT_DIR}/update.sh" >/dev/null

if git ls-remote "${AUR_FETCH_URL}" >/dev/null 2>&1; then
  git clone "${AUR_FETCH_URL}" "${WORK_DIR}/${PKGNAME}" >/dev/null 2>&1
  git -C "${WORK_DIR}/${PKGNAME}" remote set-url --push origin "${AUR_REMOTE_URL}"
else
  git init --initial-branch=master "${WORK_DIR}/${PKGNAME}" >/dev/null 2>&1
  git -C "${WORK_DIR}/${PKGNAME}" remote add origin "${AUR_FETCH_URL}"
  git -C "${WORK_DIR}/${PKGNAME}" remote set-url --push origin "${AUR_REMOTE_URL}"
fi
