...
if git -C "${WORK_DIR}/${PKGNAME}" diff --cached --quiet; then
  printf 'No changes to publish for %s\n' "${PKGNAME}"
  exit 0
fi

if git -C "${WORK_DIR}/${PKGNAME}" rev-parse --verify HEAD >/dev/null 2>&1; then
  git -C "${WORK_DIR}/${PKGNAME}" commit -m "Update ${PKGNAME}" >/dev/null 2>&1
else
  git -C "${WORK_DIR}/${PKGNAME}" commit -m "Initial import: ${PKGNAME}" >/dev/null 2>&1
fi

if ! git -C "${WORK_DIR}/${PKGNAME}" push origin master; then
  if git -C "${WORK_DIR}/${PKGNAME}" ls-remote --exit-code --heads origin master >/dev/null 2>&1; then
    git -C "${WORK_DIR}/${PKGNAME}" fetch origin master
    git -C "${WORK_DIR}/${PKGNAME}" rebase origin/master
  fi
